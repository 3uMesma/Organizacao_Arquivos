...
void funcao1(bool debug_mode){
    ...
    */
    ...
    /**
        ...
        arquivo é fechado.
    */
    arquivo_csv = fopen_r(path_csv);
    arquivo_bin = fopen_wb(path_bin);
...
void funcao2(bool debug_mode){
    ...
    */
    ...
    CABECALHO *cab= cabecalho_criar();
    cabecalho_read_bin(cab, arquivo_bin);
...
void funcao3(bool debug_mode){
    ...
    */
    ...
    /**
        ...
        arquivo é fechado.
    */
    FILE *arquivo_entrada = fopen_rb(path_entrada);
    ...
    */
    sql_CREATEINDEX(arquivo_entrada,path_saida,INDEXADOR_INDICE,campo_indexado,false,debug_mode);
...
void funcao4(bool debug_mode){
    ...
    */
    ...
    FILE *arquivo_registros = fopen_rb(path_registros);
    FILE *arquivo_indice = fopen_rb(path_indice);
    ...
    */
    CABECALHO *cab = cabecalho_criar();
    long int byteoffset_cab = cabecalho_read_bin(cab,arquivo_registros);
...
void funcao5(bool debug_mode){
    ...
    */
    ...
    if(busca_binaria_habilitada){
        fseek(arquivo_registros,0,SEEK_SET);
        removidos = sql_CREATEINDEX(arquivo_registros,path_indice,INDEXADOR_INDICE,campo_indexado,true,debug_mode);
    }
    /**
...
void funcao7(bool debug_mode){
    ...
    */
    for(int i=0;i<qtd_updates;i++){
        scanf("%d ",&q_filtros);
        resultados = sql_WHERE(arquivo_registros,arquivo_indice,cab->nroRegArq,
                        g, campo_indexado, busca_binaria_habilitada, tipo_dado,
        ...
        for(int k = 0; k < len_results; k++){
            ...
            if (tam_novo <= tam_existente){
                ...
                */
                ...
                fclose(arquivo_registros);
                fclose(arquivo_indice);
                byteoffset_add = sql_INSERTINTO(&reg, 1,INDEXADOR_INDICE, path_registros, path_indice, campo_indexado, true, debug_mode);
                ...
                arquivo_registros = fopen_rmaisb(path_registros);
                arquivo_indice = fopen_rmaisb(path_indice);

                /** Atualiza os cabecalhos */
                byteoffset_cab = cabecalho_read_bin(cab,arquivo_registros);
                //cabecalhoidx_read_bin(cab_idx,arquivo_indice);
                gerente_read_cabecalho(g,arquivo_indice,false);
            }
            /* Depois de ajeitar o arquivo registros, vamos arrumar o arquivo índice*/
            if(muda_indice){
                //printf("Tentativa de atualização no indexador do byteoffset [%ld]\n",resultados[k]);
                // indexador_remover(indexador, ChaveBusca_antiga, resultados[k]);
                // indexador_add(indexador, ChaveBusca_nova, byteoffset_novo,flags);
                gerente_remove(g,(DADO_GENERICO)ChaveBusca_antiga,resultados[k],arquivo_indice);
                gerente_add(g,(DADO_GENERICO)ChaveBusca_nova,byteoffset_novo,arquivo_indice,debug_mode);
            }
        }
...
/** FUNÇÃO 8: CREATE TABLE (ARVORE B*)*/
void funcao8(bool debug_mode){
    /** Inicializando variáveis para ler o resto da instrução*/
    char path_indice[20];
    char campo_indexado[20];
    char tipo_dado_str[20];
    char path_registros[20];
    ...
    scanf("%s %s %s %s %*c",path_registros,campo_indexado,tipo_dado_str,path_indice);

    /** Abrindo o arquivo de registros*/
    FILE *arq_registros = fopen_rb(path_registros);
    
    /** Criando o arquivo de indexação através do comando 'sql_CREATEINDEX',
     * disponível no TAD 'sql.c'. Nesse comando já é especificado como parâmetro
     * que a indexação deve ser feita usando a árvore B*, bem como espeficica
     * o arquivo de índice que será criado através do 'path_indice'.
    */
    sql_CREATEINDEX(arq_registros, path_indice, INDEXADOR_ARVOREB, 
                    campo_indexado, false, debug_mode);
    
    fclose(arq_registros);
    /** Chama o binário na tela do arquivo de índice criado*/
    binarioNaTela(path_indice);
}

/** FUNÇÃO 9: WHERE (ARVORE B*)*/
void funcao9(bool debug_mode){
    /** Inicializando variáveis para ler o resto da instrução*/
    char path_indice[20];
    char campo_indexado[20];
    char tipo_dado_str[20];
    ...
    /** Lendo as informações básicas*/
    scanf("%s %s %s %s %d",path_registros,campo_indexado,tipo_dado_str,path_indice, &qtd_buscas);
    ...
    */
    ...
    /** Abrindo os arquivos das maneiras correspondentes*/
    FILE *arq_registros = fopen_rb(path_registros);
    FILE *arq_indice = fopen_rb(path_indice);

    /** Ler o cabeçalho do arquivo de registros e criar o gerenciador de índice*/
    CABECALHO *cab = cabecalho_criar();
    long int byteoffset_cab = cabecalho_read_bin(cab,arq_registros);
    GERENTE *g = gerente_criar(INDEXADOR_ARVOREB,tipo_dado);
    ...
    */
    long int *byteofsetts_filtro=malloc(0);
    int len_Boff  = 0;
    int m; /** quantidade de filtros por busca*/
    int j; /** variável auxilia para a busca*/
    ...
    for(int b=0;b<qtd_buscas;b++){
        ...
        */
        ...
        /**
            ...
            retornar os byteoffsets filtrados
        */
        byteofsetts_filtro = sql_WHERE(arq_registros,
                                       arq_indice,
                                       cabecalho_nreg(cab),
                                       g,
                                       campo_indexado,
        ...
        */
        ...
        if(len_Boff>0){
            for(j=0;j<len_Boff;j++){
                if(byteofsetts_filtro[j]>cabecalho_proxByteoffset(cab)){continue;}
                fseek(arq_registros,byteofsetts_filtro[j],SEEK_SET);
                registro_ler_bin(arq_registros, flags, reg);
                registro_print(reg);
            }
        }else{
            printf("Registro inexistente.\n");
...
/** FUNÇÃO 10: INSERT INTO (ARVORE B*)*/
void funcao10(bool debug_mode){
    /** Inicializando variáveis para ler o resto da instrução*/
    char path_indice[20];
    char campo_indexado[20];
    char tipo_dado_str[20];
    ...
    /** Lendo os dados de todas as inserções que querem ser feitas e colocando isso em vetor de struct
    de registro*/
    REGISTRO **regs = malloc(sizeof(REGISTRO)*qtd_insercoes);
    for (int i = 0; i < qtd_insercoes; i++){
        regs[i] = registro_criar();
    ...
    /** chama o 'sql_INSERTINTO' específico para árvoreB**/
    sql_INSERTINTO(regs, qtd_insercoes, INDEXADOR_ARVOREB,
                   path_registros, path_indice, campo_indexado,
                   false, debug_mode);

    /** Após as inserções, chamar a função binário na tela para a validação
    no run codes*/
    binarioNaTela(path_registros);
    binarioNaTela(path_indice);

    /** Por último, devemos liberar a memória alocada para os registros*/
    for(int i=0;i<qtd_insercoes;i++){
        registro_free(&regs[i]);
    }
    free(regs);
